...
Next we will need to create a service. Take a look at ``service.json``{{open}}. Here our service is defined. Here we can define the service's name, ports, and other fields. Notice the `selector` in particular. This is how we define what pods we should add to this service. The selector references a label that we will apply to each pod we want to use with this service. 
...
Next we will modify the client applications `pom.xml` to include a label. Notice that the label is the same as the selector in the service definition above. 
